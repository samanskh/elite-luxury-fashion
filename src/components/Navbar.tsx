...
export const Navbar: React.FC = () => {
  ...
  return (
    <>
      ...
      <div className="bg-[#1c1917] text-[#f5f5f4] dark:bg-stone-900 dark:border-b dark:border-stone-800 text-xs py-2 px-4 text-center tracking-wide font-medium flex items-center justify-between">
        <div className="hidden md:block w-32 text-right text-stone-400">تلفن پشتیبانی: ۰۲۱-۲۲۰۰۸۸۹۹</div>
        ...
        <div className="hidden md:block w-32 text-left">
          <button 
            ...
            className="text-amber-400 hover:underline flex items-center justify-end gap-1 font-semibold"
          >
            <Sparkles className="w-3.5 h-3.5" />
            <span>مشاور استایل الیت</span>
          </button>
        </div>
      </div>
      ...
      <header className="sticky top-0 z-40 bg-[#faf9f6]/95 dark:bg-[#121212]/95 backdrop-blur-md border-b border-stone-200/80 dark:border-stone-800 transition-colors duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            {/* Right side (RTL): Mobile Hamburger & Brand Menu Navigation */}
            <div className="flex items-center gap-6">
              <button
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className="md:hidden p-2 text-stone-800 dark:text-stone-200 hover:bg-stone-100 dark:hover:bg-stone-800 rounded-lg"
                aria-label="منو"
              >
              ...
              >
                <span className="font-brand text-2xl md:text-3xl font-extrabold tracking-[0.2em] text-stone-900 dark:text-stone-100 group-hover:opacity-80 transition-opacity">
                  ELITE
                </span>
                <span className="text-[9px] tracking-[0.3em] font-light text-stone-500 dark:text-stone-400 -mt-1 uppercase">
                  ATELIER TEHRAN
                </span>
              </button>

              {/* Desktop Desktop Nav Menu */}
              <nav className="hidden md:flex items-center gap-7 mr-8 text-sm font-medium text-stone-700 dark:text-stone-300">
                <button
                  onClick={() => navigateTo('home')}
                ...
                >
                  ...
                  {isCategoryDropdownOpen && (
                    <div className="absolute right-0 top-full w-64 bg-white dark:bg-stone-900 shadow-2xl rounded-xl border border-stone-200 dark:border-stone-800 py-3 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                >
                  ...
                  <span>کالکشن پاییز و زمستان</span>
                </button>

                <button
                  onClick={() => navigateTo('about')}
                  className={`hover:text-stone-900 dark:hover:text-white transition-colors py-2 ${
                    currentPage === 'about' ? 'text-stone-950 dark:text-amber-400 font-bold' : ''
                  }`}
                >
                  درباره الیت
                </button>
                ...
                <button
                  ...
                  className={`hover:text-stone-900 dark:hover:text-white transition-colors py-2 ${
                    currentPage === 'contact' ? 'text-stone-950 dark:text-amber-400 font-bold' : ''
                  }`}
                >
                  تماس و شعب
                </button>
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              {/* AI Stylist Launcher Button */}
              <button
                onClick={() => setIsStylistOpen(true)}
                className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-500/10 to-stone-900/10 dark:from-amber-400/20 dark:to-stone-100/10 border border-amber-500/30 text-stone-900 dark:text-amber-300 text-xs font-semibold hover:opacity-90 transition-all shadow-xs"
                title="مشاور هوشمند استایل الیت"
              >
                <Sparkles className="w-3.5 h-3.5 text-amber-500 animate-pulse" />
                <span>مشاور استایل</span>
              </button>

              {/* Search Toggle Button */}
              <button
                onClick={() => setIsSearchOpen(true)}
                className="p-2.5 text-stone-700 dark:text-stone-300 hover:text-stone-900 dark:hover:text-white hover:bg-stone-100 dark:hover:bg-stone-800 rounded-full transition-colors"
                aria-label="جستجو"
              >
              ...
              <button
                ...
                className="relative p-2.5 text-stone-700 dark:text-stone-300 hover:text-stone-900 dark:hover:text-white hover:bg-stone-100 dark:hover:bg-stone-800 rounded-full transition-colors"
                aria-label="علاقه‌مندی‌ها"
              >
                <Heart className="w-5 h-5" />
                {wishlist.length > 0 && (
                  <span className="absolute top-1.5 right-1.5 bg-rose-500 text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
              ...
              >
                ...
                {cartItemCount > 0 && (
                  <span className="absolute top-1.5 right-1.5 bg-[#1c1917] dark:bg-amber-400 text-white dark:text-black text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden border-t border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900 px-4 pt-3 pb-6 space-y-3 animate-in slide-in-from-top-4 duration-200">
            ...
            <div className="border-t border-stone-100 dark:border-stone-800 pt-2">
              <div className="text-xs font-semibold text-stone-400 mb-2">دسته‌بندی‌های لوکس</div>
              {mockCategories.map(cat => (
                <button
                  key={cat.id}
                  onClick={() => {
            ...
            <div className="border-t border-stone-100 dark:border-stone-800 pt-3 space-y-2">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-stone-900 text-white dark:bg-amber-400 dark:text-black font-semibold text-sm"
              >
                <Sparkles className="w-4 h-4" />
                <span>گفتگو با مشاور استایل هوشمند الیت</span>
              </button>
